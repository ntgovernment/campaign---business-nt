...
(function (global) {
  ...
  async function renderChooseTopic(contentEl, navJson) {
    ...
    for (const q of navJson.quizzes) {
      const card = document.createElement('div'); card.className = 'card quiz-card';
      ...
      const progressWrap = document.createElement('div'); progressWrap.className = 'card-progress';
      const percent = await computeProgress(q.id);
      const circleProgress = document.createElement('div'); circleProgress.className = 'circle-progress';
  ...
  async function renderQuizPage(contentEl, quizId, pageIndex) {
    ...
    // nav
    const nav = document.createElement('div'); nav.className = 'nav-buttons';
    const back = document.createElement('button'); back.className = 'secondary'; back.textContent = 'Previous page';
    const next = document.createElement('button'); next.className = 'primary'; next.textContent = 'Next page';
  ...
  async function renderResults(contentEl, quizId) {
    ...
    try {
      ...
      const nextBtn = document.createElement('button'); nextBtn.className = 'secondary';
      ...
      prevBtn.addEventListener('click', () => {
        if (idx > 0) { const prevId = nav.quizzes[idx - 1].id; window.State.currentState.view = 'quiz'; window.State.currentState.quizId = prevId; window.State.currentState.currentPage = 0; window.State.saveStateToUrl(window.State.currentState); }
      });
      nextBtn.addEventListener('click', () => {
        if (idx < nav.quizzes.length - 1) { const nextId = nav.quizzes[idx + 1].id; window.State.currentState.view = 'quiz'; window.State.currentState.quizId = nextId; window.State.currentState.currentPage = 0; window.State.saveStateToUrl(window.State.currentState); }
      });
